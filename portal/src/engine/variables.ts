...
export const resolveVariable = (path: string, context: VariableContext): unknown => {
  ...
  const builtIns: Record<string, unknown> = {
    ...
    weather: context.weather ?? context.status?.weather,
    weather_code: context.weather?.weatherCode ?? context.status?.weather?.weatherCode,
    description: context.weather?.description ?? context.status?.weather?.description ?? payload.description,
    forecast: context.weather?.forecast ?? context.status?.weather?.forecast,
