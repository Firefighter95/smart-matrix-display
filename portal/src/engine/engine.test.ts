...
describe('layout and variable contracts', () => {
  ...
  it('keeps the weather clock inside the bezel and uses a clean code-colored summary', () => {
    const weatherLayout = createDefaultLayouts().find((layout) => layout.id === 'clock-weather');
    expect(weatherLayout?.elements.map((item) => item.id)).toEqual(['time', 'date', 'weather-summary', 'forecast-summary', 'weather-metrics']);
  ...
  it('scrolls the complete forecast and places wind direction before its speed', () => {
    ...
    expect(horizontalScrollWindow(text, 20, 249)).toBe(horizontalScrollWindow(text, 20, 0));
    expect(horizontalScrollWindow(text, 20, 250)).not.toBe(horizontalScrollWindow(text, 20, 0));
    expect(horizontalScrollWindow(text, 20, 500)).not.toBe(horizontalScrollWindow(text, 20, 250));
  });
