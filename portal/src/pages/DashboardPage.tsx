...
export function DashboardPage({ status, config, onNavigate, onClear, onSendMessage }: Props) {
  ...
  const weatherReceivedAt = weather?.observedAt ? new Date(weather.observedAt).toLocaleString('nl-NL') : undefined;
  const windMs = typeof weather?.windSpeedKph === 'number' ? `${(weather.windSpeedKph / 3.6).toFixed(1)} m/s` : '—';
  return <div className="dashboard-page">
    ...
    <div className="dashboard-grid">
      ...
      <section className="panel"><div className="panel-heading"><div><span className="eyebrow">WEATHER INPUT</span><h2>Ontvangen weerdata</h2></div><StatusBadge tone={weather ? 'success' : 'warning'}>{weather ? 'Ontvangen' : 'Geen data'}</StatusBadge></div><div className="health-list"><div><span>Conditie</span><strong>{weather?.condition || '—'}</strong></div><div><span>Temperatuur</span><strong>{typeof weather?.temperatureC === 'number' ? `${weather.temperatureC.toFixed(1)} °C` : '—'}</strong></div><div><span>Windsnelheid</span><strong>{windMs}</strong></div><div><span>Bron</span><strong>{weather?.source || '—'}</strong></div></div>{weatherReceivedAt && <div className="helper-text">Laatste snapshot: {weatherReceivedAt}</div>}<div className="quick-note"><span className="note-icon">i</span><span>Deze waarden komen rechtstreeks uit de laatste snapshot van Home Assistant. Transport gebruikt km/h; display en portal tonen m/s.</span></div></section>
