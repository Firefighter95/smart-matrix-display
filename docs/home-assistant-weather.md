...
De snapshotvelden zijn `weatherCode`, `description`, `forecast`, `warning`, `precipitationTodayProbability`, `precipitationTomorrowProbability`, `globalRadiationWm2`, `windDirection` en `sunState`. De compacte standaard Weather Clock toont weercode en omschrijving met kleur afgestemd op de code, verwachting, waarschuwing, temperatuur, windsnelheid in m/s en neerslagkans vandaag/morgen. Stralingsintensiteit, windrichting en zonstatus blijven beschikbaar voor eigen layouts en als HA-sensors.
