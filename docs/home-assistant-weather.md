...
De snapshotvelden zijn `weatherCode`, `description`, `forecast`, `warning`, `precipitationTodayProbability`, `precipitationTomorrowProbability`, `globalRadiationWm2`, `windDirection` en `sunState`. De compacte standaard Weather Clock toont weercode en omschrijving met kleur afgestemd op de code, verwachting, waarschuwing, temperatuur, windrichting vóór windsnelheid in m/s en neerslagkans vandaag/morgen. Lange regels scrollen non-blocking door het beschikbare tekstvak zodat de volledige tekst leesbaar blijft. Als de weather entity een `wind_bearing` in graden aanbiedt, zet de integratie die om naar een Nederlandse 16-punts windrichting; anders kan de optionele windrichtingssensor worden geselecteerd.
